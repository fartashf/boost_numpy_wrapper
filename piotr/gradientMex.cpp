...
/******************************************************************************/
// Create [hxwxd] cv::Mat array, initialize to 0 if c=true
cv::Mat* mxCreateMatrix3( int h, int w, int d, int id, bool c, void **I ){
  const int dims[3]={h,w,d}, n=h*w*d; int b; cv::Mat* M; int type;
  if( id==CV_32S ){ b=sizeof(int); type = CV_32S(d);}
  else if( id==CV_64F ){ b=sizeof(double); type = CV_64F(d);}
  else if( id==CV_32F ){ b=sizeof(float); type = CV_32F(d);}
  else fprintf(stderr,"Unknown data type.");
  // *I = malloc(n*b);
  // if (c)
  //     memset(*I, *I+n*b, 0);
  M = new cv::Mat(h, w, type);
  *I = M.data;
  return M;
}

// Check inputs and outputs to mex, retrieve first input I
void checkArgs( const cv::Mat *pr, int *h, int *w, int *d, int id, void **I )
{
  *h=pr->rows; *w=pr->cols; *d=pr->channels(); *I = pr->data;
  if( pr->depth()!=id ) fprintf(stderr,"I has incorrect type.");
}

// [Gx,Gy] = grad2(I) - see gradient2.m
void mGrad2( int nl, cv::Mat *pl[], int nr, const cv::Mat *pr[] ) {
  int h, w, d; float *I, *Gx, *Gy;
  checkArgs(nl,pl,nr,pr,1,2,1,1,&h,&w,&d,CV_32F,(void**)&I);
  if(h<2 || w<2) fprintf(stderr,"I must be at least 2x2.");
  pl[0]= mxCreateMatrix3( h, w, d, CV_32F, 0, (void**) &Gx );
  pl[1]= mxCreateMatrix3( h, w, d, CV_32F, 0, (void**) &Gy );
  grad2( I, Gx, Gy, h, w, d );
}

// [M,O] = gradMag( I, channel, full ) - see gradientMag.m
void mGradMag( int nl, cv::Mat *pl[], int nr, const cv::Mat *pr[] ) {
  int h, w, d, c, full; float *I, *M, *O=0;
  checkArgs(nl,pl,nr,pr,1,2,3,3,&h,&w,&d,CV_32F,(void**)&I);
  if(h<2 || w<2) fprintf(stderr,"I must be at least 2x2.");
  c = (int) mxGetScalar(pr[1]); full = (int) mxGetScalar(pr[2]);
  if( c>0 && c<=d ) { I += h*w*(c-1); d=1; }
  pl[0] = mxCreateMatrix3(h,w,1,CV_32F,0,(void**)&M);
  if(nl>=2) pl[1] = mxCreateMatrix3(h,w,1,CV_32F,0,(void**)&O);
  gradMag(I, M, O, h, w, d, full>0 );
}

// gradMagNorm( M, S, norm ) - operates on M - see gradientMag.m
void mGradMagNorm( int nl, cv::Mat *pl[], int nr, const cv::Mat *pr[] ) {
  int h, w, d; float *M, *S, norm;
  checkArgs(nl,pl,nr,pr,0,0,3,3,&h,&w,&d,CV_32F,(void**)&M);
  if( pr[1].rows!=h || pr[1].cols!=w || d!=1 ||
    pr[1].depth()!=CV_32F ) fprintf(stderr,"M or S is bad.");
  S = (float*) pr[1].data; norm = (float) mxGetScalar(pr[2]);
  gradMagNorm(M,S,h,w,norm);
}

// H=gradHist(M,O,[...]) - see gradientHist.m
void mGradHist( int nl, cv::Mat *pl[], int nr, const cv::Mat *pr[] ) {
  int h, w, d, hb, wb, nChns, binSize, nOrients, softBin, useHog;
  bool full; float *M, *O, *H, clipHog;
  checkArgs(nl,pl,nr,pr,1,3,2,8,&h,&w,&d,CV_32F,(void**)&M);
  O = (float*) pr[1].data;
  if( pr[1].rows!=h || pr[1].cols!=w || d!=1 ||
    pr[1].depth()!=CV_32F ) fprintf(stderr,"M or O is bad.");
  ...
  pl[0] = mxCreateMatrix3(hb,wb,nChns,CV_32F,1,(void**)&H);
